---
name: incus-project-nnh
description: "nnh runs in its OWN separate Incus project on nikopol-nixos, reusing default's bridges without touching rke2lab"
metadata: 
  node_type: memory
  type: project
...
nnh runs in its **own separate Incus project** on the `nikopol-nixos` remote, NOT inside
rke2lab's cluster/project. (Dependency inversion — nnh moving onto the cluster — comes LATER,
...
- Projects: `default`, `rke2lab`. → create `nnh`.
...
- `incus project create nikopol-nixos:nnh` → defaults give `features.networks=false`, so the
  project **reuses `default`'s `lan-br`** — no network duplication, no touch to rke2lab.
- A `nnh` profile, collector on a **SINGLE NIC** `lan0`→`lan-br` (the shared Wi-Fi L2), with a
  pinned `hwaddr` (stable MAC, nnh-distinct). No `vmnet0` — `vmnet-br` is internal inter-instance
  and the collector is a standalone appliance that doesn't talk to rke2 nodes.
- The collector instance = nnh's nix-built LXC image, imported into project `nnh`.
...
sub-dataset per volume/instance under `tank/nerd/incus/…`, and project scoping keeps nnh's data
separate. Rejected a dedicated `tank/nerd/nnh` dataset/pool (would give own compression/quota/
...
1. project `nnh` (features.networks=false).
2. volume `data` (pool default).
3. profile `nnh`: root disk (pool default) + NIC `lan0` (bridged, parent lan-br, pinned hwaddr,
...
5. instance `collector` (from the image, project nnh, profile nnh).
